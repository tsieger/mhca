# Demo on treatment of clusters with subthreshold size: one of subthreshold clusters consists of several subcluster.

if (exists('saveMe')&&saveMe) png('subthresh3.png',1400,1000)
opar<-par(mfcol=c(3,8),mar=c(3,2,2,1)+0.1)

set.seed(1)
n1<-10
xy2<-rbind(
    cbind(rnorm(n1),rnorm(n1,0,.02)),cbind(rnorm(n1),rnorm(n1,.3,.02)),cbind(rnorm(n1,5),rnorm(n1,0,.02)),cbind(rnorm(n1,5),rnorm(n1,.3,.02)),
    cbind(rnorm(4*n1,20),rnorm(4*n1,1,.2)),
    cbind(rnorm(4*n1,20),rnorm(4*n1,-1,.2)))
xy2[,2]<-xy2[,2]*10
...
# Mahalanobis HCA
   mh1.vm<-mhclust(  1*xy2,thresh=1/k,useR=useR,subthreshHandling='mahal')
  mh10.vm<-mhclust( 10*xy2,thresh=1/k,useR=useR,subthreshHandling='mahal')
 mh100.vm<-mhclust(100*xy2,thresh=1/k,useR=useR,subthreshHandling='mahal')
  mh1.det<-mhclust(  1*xy2,thresh=1/k,useR=useR,subthreshHandling='mahal0')
 mh10.det<-mhclust( 10*xy2,thresh=1/k,useR=useR,subthreshHandling='mahal0')
mh100.det<-mhclust(100*xy2,thresh=1/k,useR=useR,subthreshHandling='mahal0')
  mh1.eu<-mhclust(  1*xy2,thresh=1/k,useR=useR,subthreshHandling='euclid')
 mh10.eu<-mhclust( 10*xy2,thresh=1/k,useR=useR,subthreshHandling='euclid')
mh100.eu<-mhclust(100*xy2,thresh=1/k,useR=useR,subthreshHandling='euclid')
  mh1.em<-mhclust(  1*xy2,thresh=1/k,useR=useR,subthreshHandling='euclidMahal')
 mh10.em<-mhclust( 10*xy2,thresh=1/k,useR=useR,subthreshHandling='euclidMahal')
mh100.em<-mhclust(100*xy2,thresh=1/k,useR=useR,subthreshHandling='euclidMahal')
...
addMark<-function(m) {
    mtext(paste0('3',m),line=.3,adj=0,font=2,cex=1.5)
}
#
# subthreshHandling='meanvar'
#
