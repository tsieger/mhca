# Demo on treatment of clusters with subthreshold size: three elongated clusters in line.

if (exists('saveMe')&&saveMe) png('subthresh2.png',1400,1000)
opar<-par(mfcol=c(3,8),mar=c(3,2,2,1)+0.1)

set.seed(1)
xy2<-rbind(
    rnorm(50)+cbind(0,rnorm(50,0,.2)),
    rnorm(50,8)+cbind(0,rnorm(50,0,.2)),
    rnorm(50,16)+cbind(0,rnorm(50,0,.2)))
...
# Mahalanobis HCA
 ...
 mh100.vm<-mhclust(.1*xy2,thresh=1/k,useR=useR,subthreshHandling='mahal')
...
mh100.det<-mhclust(.1*xy2,thresh=1/k,useR=useR,subthreshHandling='mahal0')
...
mh100.eu<-mhclust(.1*xy2,thresh=1/k,useR=useR,subthreshHandling='euclid')
...
mh100.em<-mhclust(.1*xy2,thresh=1/k,useR=useR,subthreshHandling='euclidMahal')
...
addMark<-function(m) {
    mtext(paste0('2',m),line=.3,adj=0,font=2,cex=1.5)
}
