...
gRecursive = TRUE, ##<< boolean. If \code{TRUE}, clustering of the
## apriori clusters is performed in a recursive manner in space
## `O(m^2)', where `m' is the apriori cluster size, and the apriori
## clusters get clustered subsequently, in space `O(g^2)', where `g' is
## the number of apriori clusters. If \code{FALSE}, the clustering of
## the apriori clusters is performed as part of the whole clustering,
## so that it requires `O(n^2)' space, where `n' is the number of all
## observations. The best (and the default) option is thus \code{TRUE}.
...
## data set (internal parameter)
    ...
    if (!is.null(g)) {
        ...
        if (gRecursive) {
            ...
            # apriori clusters get clustered as part of the whole clustering,
            # we just need to set distance matrix entries that do not correspond
            # to distances within apriori clusters to Inf

            # indices into the distance matrix where pairs of points g reside
            gDistIdx<-integer(n*(n-1)/2)
            gDistIdxCnt<-0
            for (gti in 1:length(gt)) {
    ...
    if (useR) {
        ...
        rv<-mhclust_Rimpl(x,thresh,scale,quick,normalize,g,gMergingCount,gDistIdx,verb,nFull,nLeft,distX,centroid,members,invcov,detsSqrt,weightFactor,clusterId,clusterSize,merging,height)
    } else {
        if (verb) cat('using C implementation\n')
        rv<-mhclust_Cimpl(x,thresh,scale,quick,normalize,g,gMergingCount,gDistIdx,verb,nFull,nLeft,distX,centroid,members,invcov,detsSqrt,weightFactor,clusterId,clusterSize,merging,height)
    }
